...
fun WtContext.toTransportUb(): IResponse = when (val cmd = command) {
...
fun WtContext.toInit(): IResponse = InitResponse(
    result = state.toResult(),
    errors = errors.toErrors()
)

fun WtContext.toSearch(): IResponse = SearchResponse(
    result = state.toResult(),
    errors = errors.toErrors(),
    ubs = ubsResponse.toTransportUbs()
)

fun WtContext.toDelete(): IResponse = DeleteResponse(
    result = state.toResult(),
    errors = errors.toErrors(),
    ub = response.toTransportUb()
)

fun WtContext.toUpdate(): IResponse = UpdateResponse(
    result = state.toResult(),
    errors = errors.toErrors(),
    ub = response.toTransportUb()
)

fun WtContext.toRead(): IResponse = ReadResponse(
    result = state.toResult(),
    errors = errors.toErrors(),
    ub = response.toTransportUb()
)
