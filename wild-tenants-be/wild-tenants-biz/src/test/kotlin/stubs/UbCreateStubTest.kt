...
class UbCreateStubTest {
    ...
    @Test
    fun `should create Ub`() = runTest {

        val context = WtContext(
            command = WtCommand.CREATE,
            state = WtState.NONE,
            workMode = WtWorkMode.STUB,
    ...
    fun `should return error when meter reading is empty`() = runTest {
        val context = WtContext(
            ...
            state = WtState.NONE,
            workMode = WtWorkMode.STUB,
            stubCase = WtStubs.BAD_METER_READING,
            request = WtUb(
                id = id,
